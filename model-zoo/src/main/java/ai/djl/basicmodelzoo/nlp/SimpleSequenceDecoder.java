...
public class SimpleSequenceDecoder extends Decoder {
     ...
     */
    public SimpleSequenceDecoder(RecurrentBlock recurrentBlock, int vocabSize, String eosToken, String bosToken) {
    ...
    public NDList predict(ParameterStore parameterStore, NDList inputs) {
        ...
        if (inputShape.get(0) != 1 && inputShape.get(1) != 1) {
            throw new IllegalArgumentException("Input to predict must be of shape (1, 1, <embeddingSize>");
        }
        ...
        while (!token.equals(bosToken) && i < 10) {
            ...
            //System.out.println("Input to TextEmbedding" + outputList.head().argMax(2).getShape());
            token = trainableTextEmbedding.unembedText(outputList.head().argMax(2).toType(DataType.INT32, false)).get(0);
