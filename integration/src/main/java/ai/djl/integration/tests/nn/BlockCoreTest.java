    ...
    public void testRNNTanh() throws IOException, MalformedModelException {
        ...
        try (Model model = Model.newInstance(config.getDevices()[0])) {
            ...
            try (Trainer trainer = model.newTrainer(config)) {
                ...
                trainer.initialize(inputShape);
                NDManager manager = trainer.getManager();
                NDArray data = manager.create(new float[] {1, 2, 3, 4, 5, 6, 7, 8}).reshape(inputShape);
                NDArray labels = manager.create(new float[] {1, 2, 3, 4, 5, 6, 7, 8}).reshape(inputShape);
        ...
        try (Model model = Model.newInstance(config.getDevices()[0])) {
            ...
            try (Trainer trainer = model.newTrainer(config)) {
                ...
                NDArray expected =
                        manager.create(
                                new float[] {
                                        00.7615f, 0.7615f, 0.7615f, 0.7615f, 0.964f, 0.964f, 0.964f, 0.964f
                                },
                                new Shape(1, 2, 4));
                Assertions.assertAlmostEquals(result.head(), expected);
                Assertions.assertAlmostEquals(result.size(), 2);
        ...
        try (Model model = Model.newInstance(config.getDevices()[0])) {
            ...
            try (Trainer trainer = model.newTrainer(config)) {
                ...
                NDArray expected =
                        manager.create(
                                new float[] {
                                        4.54187393e-05f, 4.54187393e-05f, 4.54187393e-05f, 4.54187393e-05f, 4.54187393e-05f, 4.54187393e-05f, 4.54187393e-05f, 4.54187393e-05f
                                },
                                new Shape(1, 2, 4));
                Assertions.assertAlmostEquals(result.head(), expected);
